...
if plotCurve:
	...
	for cnt in range(num_iter):
		...
		adagrad += grad ** 2
		theta = theta - (lr / np.sqrt(adagrad + eps)) * grad
	...
	#normalize features
	test_mean = np.mean(testData, axis=0)
	test_std = np.std(testData, axis=0)
...
for cnt in range(1,num_iter):
	...
	train_cost.append(computeCost(tmp_xTrain, tmp_yTrain, theta))
	val_cost.append(computeCost(x_val, y_val, theta))
